...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/07/03 11:51:04 by fguarrac          #+#    #+#             */
/*   Updated: 2019/07/30 15:33:19 by fguarrac         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	while (link_new->btspec < link->btspec && link->next != NULL)
		link = link->next;
	if (link_new->btspec == link->btspec)
		while (ft_strcmp_lex(link_new->name, link->name) > 0
				&& link_new->btspec == link->btspec
				&& link->next != NULL)
			link = link->next;
	if (link->next == NULL
...
{
	while (link_new->btspec == link->btspec
			&& ft_strcmp_lex(link_new->name, link->name) > 0
			&& link->next != NULL)
		link = link->next;
	if (link->next == NULL
...
static t_link	*ft_make_link(char *name, char *path)
{
	...
	link->btspec = link->file_stat.st_mtimespec.tv_sec;
	link->next = NULL;
	link->prev = NULL;
	return (link);
}
...
static void		ft_link_elem(t_link *link_new, t_link **head)
{
	...
	else if (link_new->btspec > link->btspec)
	{
		link_new->next = link;
		link->prev = link_new;
	}
